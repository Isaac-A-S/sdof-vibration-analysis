...
This form shows that the response shape is governed primarily by $\zeta$. The dimensional values of $m$ and $k$ set the time and force scales.
...
The exponential factor sets the decay rate. Its half-life is
...
When $v_0=0$, this response returns to equilibrium without crossing it. It is the fastest monotonic response in the linear model.
